    ...
    public void run(ApplicationArguments args) {
        ...
        db.sql(CREATE_LECTURA).fetch().rowsUpdated()
                ...
                .then(db.sql("CREATE INDEX IF NOT EXISTS idx_lectura_sensor_ts ON lectura (sensor_id, ts DESC)")
                        .fetch().rowsUpdated())
                // FIX-0004: calidad del dato + severidad nullable (lectura ERROR_SENSOR no evaluada)
                .then(db.sql("ALTER TABLE lectura ADD COLUMN IF NOT EXISTS calidad VARCHAR(16) NOT NULL DEFAULT 'OK'")
                        .fetch().rowsUpdated())
                .then(db.sql("ALTER TABLE lectura ALTER COLUMN severidad DROP NOT NULL")
                        .fetch().rowsUpdated())
                // FIX-0006 BR-008: secuencia del emisor (NULL en payloads legados sin sequence)
                .then(db.sql("ALTER TABLE lectura ADD COLUMN IF NOT EXISTS secuencia BIGINT")
                        .fetch().rowsUpdated())
